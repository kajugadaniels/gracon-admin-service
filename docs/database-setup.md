...
Use this guide when setting up `api/admin` after cloning the project.

## Ownership

`api/admin` uses the shared database at runtime, but it does not own migrations.
...
```sql
CREATE ROLE gracon_admin_app LOGIN PASSWORD 'replace_with_admin_password';
```

If it already exists:

```sql
ALTER ROLE gracon_admin_app WITH PASSWORD 'replace_with_admin_password';
```
...
```env
DATABASE_URL=postgresql://gracon_admin_app:password@ep-example-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require
```
